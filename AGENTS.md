...
- 目标状态能力清单维护在 `VISION.md`；新增 Skill 或调整路线时，应先确认没有偏离该文件定义的产品方向。
...
- 插件版本写在 `.codex-plugin/plugin.json`；每个已发布 Skill 必须在 `SKILL.md` frontmatter 中声明独立 `version`，具体规则见 `VERSIONING.md`。
